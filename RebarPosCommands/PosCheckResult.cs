    ...
    public abstract class PosCheckResult
    {
        ...
        public static List<PosCheckResult> CheckAllInSelection(IEnumerable<ObjectId> items, bool checkErrors, bool checkWarnings)
        {
            ...
            if (checkErrors)
            {
                ...
                List<int> standardDiameters = PosSettings.Current.StandardDiameters;

                // Maximum bar length
                double maxLength = PosSettings.Current.MaxBarLength;
                ...
                foreach (PosCopy copy in pliste)
                {
                    if (PosShape.Shapes[copy.shapename].IsUnknown)
                    {
